...
### CHECK-11: candidate_pool degenerate (< 3 unique symbols)

- `N/A` (?): {'metric': 'candidate_pool_random unique_symbols', 'value': 1, 'threshold': 3, 'reason': 'candidate_pool has only 1 unique symbol(s) — baselines degenerate to fixed symbol, not random control'}
